...
void mcp23017_task(void *pvParameters)
{
    ...
    gpio_set_direction(INTA_GPIO, GPIO_MODE_INPUT);
		gpio_set_direction(INTB_GPIO, GPIO_MODE_INPUT);

    gpio_set_intr_type(INTA_GPIO, GPIO_INTR_ANYEDGE);
		gpio_set_intr_type(INTB_GPIO, GPIO_INTR_ANYEDGE);

    gpio_isr_handler_add(INTA_GPIO, mcp_inta_handler, NULL);
    gpio_isr_handler_add(INTB_GPIO, mcp_intb_handler, NULL);
